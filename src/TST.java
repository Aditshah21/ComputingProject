...
//    }
    ...
    public static void main(String[] args) {
        In myfiles = new In("Protein.txt");
        String mytext = myfiles.readAll();
        
        
        
        BruteForceMatch brt = new BruteForceMatch();
//        for( int i=0; i<txt.length(); i++ ) {
     	   
        
        TST<Integer> st = new TST<Integer>();
        StringTokenizer StrTK=new StringTokenizer(mytext, "[ -.,]");
        int i=0;
        int offset=0;
        int first_occurence;
        String str="";	
        while(StrTK.hasMoreTokens()) {
        	str = StrTK.nextToken();
        	first_occurence = brt.search1(str, mytext);
        	System.out.println(str);
        	st.put(str, first_occurence);
        }
        ...
        key = "I";  
        StdOut.println("key = "+key+", value = "+ st.prefixMatch(key));
